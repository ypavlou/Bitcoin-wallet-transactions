...
void findPayments( bucketListNode** senderHashtable,char* walletID,int senderNumOfEntries,int bucketEntries){
    bucket* found =findInBuckets(findInHashTable(senderHashtable, walletID, senderNumOfEntries),walletID, bucketEntries);
    if(found ==NULL ){
    ...
    printf("User's %s successful payments are:\n",walletID);
    printList(found->transactionList);
    printf("User's %s total payments are:%d\n\n",walletID,total);
    printf("\n");
}
void findPaymentsTime(bucketListNode** senderHashtable,char* walletID,int senderNumOfEntries,int bucketEntries,char* t1,char* t2){
    char delim[] = ":";
    ...
    bucket* found =findInBuckets(findInHashTable(senderHashtable, walletID, senderNumOfEntries),walletID, bucketEntries);  //find the walletID in hastable
    if(found ==NULL ){
        printf("User %s not register cannot display payments.\n",walletID);
        exit(1);
    }
...
void findPaymentsDate(bucketListNode** senderHashtable,char* walletID,int senderNumOfEntries,int bucketEntries,char* date1,char* date2){
    char delim[] = "-";
...
void findPaymentsDateTime(bucketListNode** senderHashtable,char* walletID,int senderNumOfEntries,int bucketEntries,char* t1,char *t2,char* date1,char* date2){
    char delim[] = ":";
    ...
    while (temp!=NULL){ //parse transaction list
        ...
        ptr = strtok(NULL, delim2);
        y3 = (int) strtol(ptr, (char **)NULL, 10);
        if(h3>=h1 && h3<=h2 && mo3>=min1 && mo3<=min2) {
            if (y3 >= y1 && y3 <= y2) {
                if (m3 >= m1 && m3 <= m2) {
                    if (d3 >= d1 && d3 <= d2) {
                        printf("%s %s %s %d %s %s\n", (temp->data).transactionID, (temp->data).senderWalletID,(temp->data).receiverWalletID, (temp->data).value, (temp->data).date, (temp->data).time);
